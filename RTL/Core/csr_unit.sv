...
module csr_unit (
    ...
    always_ff @(posedge clk) begin
        if (!start) begin
            ...
            csr.mcycleh                     <= csr.mcycleh   + ((csr.mcycle == 32'hFFFF_FFFF) ? 32'd1 : 32'd0);
            csr.minstret                    <= csr.minstret  + (instret ? 32'd1 : 32'd0);
            csr.minstreth                   <= csr.minstreth + ((instret && (csr.minstret == 32'hFFFF_FFFF)) ? 32'd1 : 32'd0);
